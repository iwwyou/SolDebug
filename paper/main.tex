...
\section{The design of SolQDebug}
    ...
    To make the architecture concrete, we walk through a running example using the burn function
    from Listing~\ref{lst:grammar-min}. We illustrate both incremental source edits (§3.2.1) and
    batch annotations (§3.2.2).
...
\end{figure*}
        ...
        SolQDebug maintains three types of CFG nodes:

\begin{itemize}
  \item \textbf{Basic nodes} store statement sequences and the abstract environment after execution.
  \item \textbf{Condition nodes} store predicates without updating the environment.
  \item \textbf{Branch refinement} prunes environments along each edge: true successors refine
        intervals under the predicate, false successors under its negation.
\end{itemize}

        When Step~9 arrives, SolQDebug processes it as follows:

\begin{enumerate}
  \item The interactive parser recognizes \texttt{totalSupply -= delta;} as an assignment (§3.3).
  \item The system locates the insertion point using the edit context and existing CFG—in this case,
        the join after the if/else (§3.5).
  \item At the join, the least upper bound (\(\sqcup\)) of branch environments is computed. If loops
        are present, a localized fixpoint is computed first (§3.5).
  \item A new node is created with the assignment evaluated in the joined environment. Edges are
        rewired from branch leaves through the join to the new node.
  \item All nodes reachable from the insertion are reinterpreted to propagate updated environments.
\end{enumerate}

        This design reuses path-local computations up to the leaves while maintaining semantic
        correctness. The same procedure handles out-of-order edits (e.g., adding an else block later)
        by applying the same insertion-point search, join handling, and edge rewiring.
...
Batch annotations allow developers to specify initial state and parameters as interval values and
obtain line-level results by reinterpreting the pre-built CFG in a single pass. Variables without
annotated ranges remain at \(\top\), making batch annotations essential for meaningful debugging
feedback.

We annotate \texttt{burn} from Listing~\ref{lst:grammar-min} with
//@StateVar \texttt{balances[msg.sender]} = \([100,200]\) and
//@LocalVar \texttt{amount} = \([50,150]\), and set \(\textit{totalSupply} = 1000\).
This makes \(\textit{bal} \ge \textit{amount}\) partially true, allowing both branches to execute.

Annotation blocks are delimited by \verb|//@Debugging BEGIN| and \verb|//@Debugging END|, with each
line assigning an abstract value to a target L-value. Targets include global, state, and local
variables, as well as nested accesses like \(\texttt{balances[addr]}\). Values are type-checked and
normalized to their respective domains (intervals for integers, symbolic sets for addresses).

When a batch block is encountered, \textsc{SolQDebug} (i) parses and type-checks each annotation,
(ii) snapshots the current abstract memory and overlays the annotated values, (iii) reinterprets the
function's CFG in a single pass with branch pruning and loop fixpoints, and (iv) restores the
snapshot to isolate successive runs. The Snapshot Manager coordinates this process.

Batch annotations do not alter the CFG structure—only the initial environment changes. The same
branch pruning, join, and fixpoint rules from §3.2.1 apply. On the pre-built CFG
(Figure~\ref{fig:solqdebug-cfg}), a batch run traverses from entry to exit in a single pass.

The annotations yield \(\textit{bal} \in [100,200]\) and \(\textit{amount} \in [50,150]\). Since
\(\textit{bal} \ge \textit{amount}\) is only partially true, both branches are reachable. After
branch pruning, the true path produces \(\delta \in [50,150]\) and \(\texttt{balances[msg.sender]}
\in [0,150]\), while the false path sets \(\delta := 0\) and leaves \(\texttt{balances[msg.sender]}
\in [100,200]\). At the join:
\[
\delta \in [0,150],
\qquad
\texttt{balances[msg.sender]} \in [0,200].
\]
Finally, \(\textit{totalSupply} -\!= \delta\) yields \([1000,1000] - [0,150] = [850,1000]\). This
demonstrates how branch pruning and joining prevent unnecessary interval blow-up while
conservatively aggregating both paths.

Containers (arrays, mappings, structs) remain at \(\top\) until accessed or annotated. Here,
\(\texttt{balances[msg.sender]}\) is concretized by annotation and propagated through branches.
State variables with explicit initial values start from fixed intervals.

In summary, batch annotations enable developers to specify initial state declaratively and explore
program behavior in a single pass over the pre-built CFG. Formal details appear in §3.3 and §3.5.
...
parser extends the official Solidity grammar (defined in \texttt{Solidity.g4}~\cite{antlr}) with
additional entry rules that accept isolated code fragments based on the current editing context. Each
fragment is parsed into a syntactically well-formed subtree suitable for incremental analysis without
requiring a complete source file.

The interactive parser is built upon ANTLR4~\cite{antlr} and introduces seven specialized entry
rules for Solidity program fragments, plus one dedicated rule for batch-annotation blocks. The
Context Analyzer determines the appropriate entry rule by inspecting the surrounding source context
(e.g., whether the cursor is inside a function body, at top level, or within a struct definition) and
selecting the most specific rule. This design allows developers to write and analyze code
incrementally—statement by statement—while maintaining full syntactic consistency with the Solidity
specification.

Table~\ref{tab:interactive-rules} summarizes the entry rules. Each rule handles specific code
fragments encountered during live editing. For instance, when a developer types a function header
followed by an auto-inserted closing brace, \texttt{interactiveSourceUnit} accepts the skeleton
definition. As statements are added inside the body, \texttt{interactiveBlockUnit} parses them
individually. Two-phase constructs such as enums, structs, \texttt{if}/\texttt{else}, and
\texttt{try}/\texttt{catch} are handled by dedicated rules that recognize when the first part (e.g.,
\texttt{enum Status \{\}}) has been declared and the second part (e.g., member items
\texttt{Pending, Shipped}) is being added incrementally.
...
\midrule
\texttt{interactiveSourceUnit} & Top-level declarations: functions, contracts, interfaces, libraries, state variables, pragmas, imports \\
\texttt{interactiveEnumUnit} & Enum member items added after the enum shell is defined \\
\texttt{interactiveStructUnit} & Struct member declarations added after the struct shell is defined \\
\texttt{interactiveBlockUnit} & Statements and control-flow skeletons inside function bodies \\
\texttt{interactiveDoWhileUnit} & The \texttt{while} tail of a \texttt{do\{...\}} loop \\
\texttt{interactiveIfElseUnit} & \texttt{else} or \texttt{else if} branches following an \texttt{if} \\
\texttt{interactiveCatchClauseUnit} & \texttt{catch} clauses following a \texttt{try} statement \\
\texttt{debugUnit} & Batch-annotation lines (\texttt{@StateVar}, \texttt{@LocalVar}) \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Representative example.}
Consider the \texttt{burn} function from Listing~\ref{lst:grammar-min}. When the developer first
types the function header, the parser invokes \texttt{interactiveSourceUnit} to accept:
\begin{lstlisting}[language=Solidity,basicstyle=\ttfamily\small]
function burn(uint256 amount) public onlyOwner {
}
\end{lstlisting}
As statements are added line-by-line inside the body, \texttt{interactiveBlockUnit} parses each one:
\begin{lstlisting}[language=Solidity,basicstyle=\ttfamily\small]
uint256 bal = balances[msg.sender];
uint256 delta;
\end{lstlisting}
When the developer types an \texttt{if} skeleton, \texttt{interactiveBlockUnit} matches it as
\texttt{interactiveIfStatement}:
\begin{lstlisting}[language=Solidity,basicstyle=\ttfamily\small]
if (bal >= amount) {
}
\end{lstlisting}
Later, if an \texttt{else} branch is added, \texttt{interactiveIfElseUnit} recognizes it and
attaches it to the preceding \texttt{if} using the construct stack maintained by the Context
Analyzer.

\noindent\textbf{Batch annotations.}
The \texttt{debugUnit} rule parses annotation lines that specify initial abstract values for state
and local variables. These annotations enable developers to configure symbolic input scenarios
without deploying contracts. For example:
\begin{lstlisting}[language=Solidity,basicstyle=\ttfamily\small]
// @Debugging BEGIN
// @StateVar balances[msg.sender] = [100,200]
// @LocalVar amount = [50,150]
// @Debugging END
\end{lstlisting}
Each directive assigns an interval or symbolic value to a variable. The parser supports nested access
patterns (e.g., \texttt{balances[msg.sender]}, \texttt{user.balance}, \texttt{arr[i].field}) and
various value types (integer intervals, symbolic addresses, boolean values). Type checking is
performed at parse time, and out-of-bounds intervals are clamped to the declared type's valid range
with a warning. The Snapshot Manager (§3.4.3) consumes these annotations to overlay the initial
abstract environment before function reinterpretation.

The complete grammar specification, including production rules and formal syntax for all entry points,
is provided in Appendix~\ref{appendix:interactive-grammar}. Implementation details such as
context-stack management and entry-rule selection algorithms are also documented there.
...
condition(false) \(\rightarrow\) loop exit; the loop exit reconnects to the original successors.
...
This mechanism is \emph{annotation-aware} without requiring special logic: when debug annotations materialize array lengths, mapping sizes, or function parameters, the evaluator naturally computes tighter intervals for condition operands, raising $\tau$ and deferring widening. Conversely, unannotated state expressions remain $\bot$, forcing the conservative default and triggering early widening (§\ref{sec:eval}, RQ3).
