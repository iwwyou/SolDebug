...
real-world benchmark for smart contract analysis. From 3,345 Solidity files
using \texttt{>=0.8.0}, we randomly sample approximately 10\% from each of
three size brackets (1--10 KB, 11--20 KB, and over 20 KB), yielding 128
contracts. After filtering out logic-free
functions (e.g., those containing only assignments or return statements),
we select 30 representative contracts using \emph{complexity-driven
stratified sampling} to ensure comprehensive coverage of debugging scenarios
encountered in real-world Solidity development. The selected contracts
range from 7 to 59 lines of code (average: 24 LOC), spanning fundamental
DeFi patterns such as token transfers with custom logic, staking/vesting
mechanisms, liquidity pool operations, oracle data processing, and
marketplace transactions. Our selection criteria focused on three dimensions
that directly impact debugging complexity: (1) \textbf{computational
complexity}---including percentage/fee calculations with varying precision
requirements, time-based vesting or locking mechanisms with multi-step
calculations, and accumulation operations with overflow protection;
(2) \textbf{data structure complexity}---including structs with multiple
fields, nested mappings with multi-level override hierarchies, dynamic
arrays requiring iteration, and mapping-to-struct patterns with conditional
lookups; and (3) \textbf{control flow complexity}---including loops with
varying termination conditions, nested conditionals with early returns,
and modifier-based access control affecting variable scopes.
Table~\ref{tab:benchmark-dataset} lists all 30 benchmark contracts with
their source files, target functions, and line ranges.
