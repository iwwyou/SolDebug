...
\begin{enumerate}
  ...
  \item \textbf{Snapshot and overlay.} The unannotated abstract memory is saved, and the annotated
  ...
  \item \textbf{Restore snapshot.} After analysis, the unannotated memory is restored.
\end{enumerate}
...
\begin{itemize}
  ...
  \item LocalVar assigns values to function parameters
\end{itemize}
...
We first describe the hierarchical organization of CFGs at the contract and function levels (\S3.3.1).
We then explain how individual statements are incrementally spliced into the function-level CFG (\S3.3.2),
how the insertion site is determined (\S3.3.3), and how reinterpretation propagates abstract values
only through affected regions (\S3.3.4).
...
\begin{itemize}
  ...
  \item \textbf{If statement} creates a \textsc{condition node}, true/false \textsc{basic nodes}, and an
        ...
        node via the least upper bound operation.
  ...
  \item \textbf{Else-if clause} replaces the preceding false branch with a new \textsc{condition node}
        ...
        and continues to the outer join.
  ...
  \item \textbf{Else clause} replaces the false branch node of the preceding \texttt{if}/\texttt{else if}
        ...
        negated condition and merges at the join.
  ...
  \item \textbf{Return statement} is inserted with its edge rewired to the function's unique \textsc{return exit},
        detaching original successors. The environment flows directly to the return exit.
  ...
  \item \textbf{Require statement} creates a \textsc{condition node} with the true edge connecting to a
        ...
        with refinement based on the predicate to the continuation or error exit.
\end{itemize}
...
\midrule
\texttt{Variable decl} & basic node & (none) & \texttt{if} & condition node & join node \\
\texttt{Assignment} & basic node & (none) & \texttt{else if} & condition node & join node \\
...
Simple statements index their start line only: most map to a basic node, while \texttt{require} maps to a condition node.
...
The complete implementation supports all Solidity constructs~\citep{solqimpl}.
...
\begin{itemize}
  \item \textbf{Line 1--3 (initialization):} Retrieves nodes at or before line \(L\) to initialize the Breadth-First Search (BFS) queue. These nodes include the join node of the preceding conditional, which serves as the starting point for backward traversal. If no nodes are found, the dependent context is invalid.
...
\begin{algorithmic}[1]
\Require CFG $G=(V,E)$, insertion position at line $L$
...
\begin{itemize}
  \item \textbf{Line 1 (find post node):} \textsc{FindPostNode}(\(L\)) retrieves the first node after line \(L\).
...
an adaptive threshold $\tau$ that defers widening. When debug annotations provide precise interval
bounds (e.g., for array lengths, parameter values), the analyzer infers tighter intervals for condition operands,
...
\begin{itemize}
  \item \textbf{Line 1 (identify loop nodes and pre-loop environment):} Collect all nodes within the loop body. Compute $Start$ by joining environments from all loop-entry predecessors (excluding back edges).

  \item \textbf{Line 2 (\textsc{EstimateIterations}):} Evaluate both operands of the loop condition (e.g., \texttt{i < array.length}) in the pre-loop environment $Start$. For comparison operators ($<, \le, >, \ge$), compute $\tau$ as the difference between operand bounds: e.g., given \texttt{i = [0,0]} and \texttt{array.length = [10,10]}, compute $\tau = 10 - 0 = 10$. When annotations constrain values to precise intervals, this yields tight thresholds; otherwise, a conservative default (typically 2) is used.
...
for addresses, and on-demand materialization for composite types
