...
practical use. To address this challenge, this paper presented \texttt{SolQDebug}, the first
...
across multiple execution paths. \texttt{SolQDebug} was evaluated on 30 real-world functions
...
demonstrated that \texttt{SolQDebug} enabled interactive debugging for Solidity development.}
...
\section{Introduction}\label{sec1}
    ...
    This paper presented \texttt{SolQDebug}, a source-level interactive Solidity debugger powered by abstract interpretation (AI) with interval domain.
    Unlike traditional debugging workflows that require compilation and deployment, \texttt{SolQDebug} provided live feedback as developers typed, helping them understand program behavior at the statement level.
    To achieve this goal, \texttt{SolQDebug} built on two core ideas.
    ...
    To validate these design choices, \texttt{SolQDebug} was evaluated on 30 real-world functions from DAppSCAN~\citep{dappscan}.
    ...
    \begin{itemize}
      ...
      \item \texttt{SolQDebug} was evaluated on 30 real-world functions from DAppSCAN, demonstrating a 350$\times$ median speedup over Remix IDE. The evaluation demonstrated that overlapping annotation patterns improve precision in common smart contract patterns and provided guidance on annotation design.
    \end{itemize}
...
\FloatBarrier
    ...
    \texttt{SolQDebug} addresses the two root causes of Solidity's debugging inefficiency through an integrated design.
    ...
    setup, and bytecode tracing. Each of these stages incurs significant latency. \texttt{SolQDebug}
    ...
    transactions. Both approaches disrupt iteration. \texttt{SolQDebug} introduces batch annotations
...
\begin{figure*}[t]
  ...
  \vspace{0.3cm}
  \caption{\texttt{SolQDebug} architecture}
  \label{fig:solqdebug-arch}
\end{figure*}

\section{The design of SolQDebug}
    \texttt{SolQDebug} is designed to analyze single-contract, single-transaction Solidity functions
    ...
    \subsection{System Architecture}
        Figure~\ref{fig:solqdebug-arch} illustrates the overall architecture of \texttt{SolQDebug}.
        ...
        Following analysis, \texttt{SolQDebug} produces a line-level summary showing computed intervals
...
\texttt{SolQDebug} accepts two kinds of code fragments as input:
...
As the developer types each fragment, \texttt{SolQDebug} incrementally extends the CFG
...
In this example, we annotate \texttt{balances[msg.sender]} with the interval \([100,200]\)
        ...
        When \texttt{SolQDebug} processes a batch annotation block, it follows this pipeline:
...
\begin{enumerate}
  ...
  \item \textbf{Single-pass analysis.} \texttt{SolQDebug} re-analyzes the pre-built CFG with
...
\texttt{SolQDebug} validates that continuation rules have their required antecedent structure,
...
\begin{itemize}
  ...
  \item \texttt{loop exit node}: The false branch that exits a loop when the guard condition fails.
  \item \texttt{return node}: A statement node whose outgoing edge is immediately rewired to the function's unique \texttt{return exit}.
  \item \texttt{error exit}: The function's unique exceptional exit (targets the exceptional path via \texttt{revert}, \texttt{require}, or \texttt{assert} failures).
...
\texttt{SolQDebug} organizes control flow at two levels. At the contract level, a
...
\begin{itemize}
  ...
  \item \textbf{Return statement} is inserted with its edge rewired to the function's unique \texttt{return exit},
  ...
  \item \textbf{Require statement} creates a \texttt{condition node} with the true edge connecting to a
        continuation node and the false edge pointing to the \texttt{error exit}. The environment propagates
        with refinement based on the predicate to the continuation or error exit (Figure~\ref{fig:new-require}).
\end{itemize}

These construction patterns enable \texttt{SolQDebug} to build the CFG incrementally
...
entire graph in a single pass. In contrast, \texttt{SolQDebug} must handle partial code edits
...
\texttt{SolQDebug} provides instant feedback on source code edits by propagating updates only along affected CFG paths, avoiding full re-analysis. When the user inserts statements, new nodes are spliced into the CFG. Incremental reinterpretation then propagates updates from these insertion points as seed nodes.
While for debug annotations, \texttt{SolQDebug} performs full interpretation from the function entry node, ensuring all inspection points receive complete abstract states.
...
\Function{IncrementalInterpretation}{$G, S$}
  ...
  \While{$wl\neq\langle\rangle$}
    ...
    \State $\hat{\sigma}_{\mathrm{in}}\gets \bigsqcup_{p\in\texttt{Pred}(G, n)} \texttt{RefineByCondition}(p,n)$ \Comment{join predecessors with path refinement}
...
\begin{itemize}
  ...
  \item \textbf{Incoming environment computation (Lines 11--12):} Compute incoming environment $\hat{\sigma}_{\mathrm{in}}$ by joining outputs from all predecessors. For condition nodes, \texttt{RefineByCondition} narrows operand intervals based on the edge truth label (true/false branch), pruning infeasible paths.
...
\texttt{SolQDebug} computes sound over-approximations of variable ranges using interval domains
...
\section{Evaluation}
To evaluate the benefits of \texttt{SolQDebug} in real-world development scenarios,
we structure our empirical analysis around three key questions:

\begin{itemize}
  \item \textbf{RQ1 -- Responsiveness}:\;%
        How much does \texttt{SolQDebug} reduce debugging latency compared to Remix?
  ...
  \item \textbf{RQ3 -- Loops}:\;%
        How does \texttt{SolQDebug}'s analysis approach affect precision in loop structures?
\end{itemize}

To answer these questions, we conducted experiments under controlled conditions.

\subsection{Experimental Setup}
\texttt{SolQDebug} was evaluated on single-contract, single-transaction functions
...
\texttt{SolQDebug} is implemented in Python~3.10 and operates directly on Solidity
...
stepping through the entire execution using the "step forward" button. For \texttt{SolQDebug}, it
...
instruction for each test input, \texttt{SolQDebug} uses AI with
...
\texttt{SolQDebug} maintains consistent latency regardless of test-case width \(\Delta\),
...
Although \texttt{SolQDebug} is designed for interactive use within a Solidity editor, all
...
\begin{figure}[t]
  ...
  \caption{Debugging latency comparison between \texttt{SolQDebug} and Remix across varying
  ByteOp counts and test-case widths \(\Delta\). \texttt{SolQDebug} maintains consistent
...
120 total measurements for \texttt{SolQDebug}. For Remix, each function was measured once per
...
In contrast, \texttt{SolQDebug} completed analysis in 0.03--5.09 seconds (median: 0.15s)
across all 120 measurements. \texttt{SolQDebug}'s latency remains nearly constant
...
time. \texttt{SolQDebug} eliminates these preparatory steps entirely, enabling immediate feedback
during code editing.
...
\textbf{Answer to RQ1:}
\texttt{SolQDebug} achieves sub-second debugging latency (median: 0.15s), providing
...
input space size, \texttt{SolQDebug}'s performance through abstract interpretation does not depend on
...
However, \texttt{SolQDebug} conservatively models uninitialized variables as $\top$ (unknown), causing \texttt{total} to remain $\top = [0, 2^{256}-1]$ regardless of the precise loop bound.
...
Once the widening threshold is exceeded, \texttt{SolQDebug} widens \texttt{resultIndex} to $[0, \infty]$.
...
\begin{minipage}{0.96\linewidth}
\textbf{Answer to RQ3:}
\texttt{SolQDebug} improves loop analysis precision for constant-bounded and annotation-enabled dynamic loops.
...
We compared \texttt{SolQDebug} only against Remix IDE,
...
In this work, debugging refers to a developer-led, interactive exploration activity that occurs during code editing, before deployment.
...
\texttt{SolQDebug} achieved 350$\times$ median speedup over Remix IDE (RQ1),
...
\section{Related Works}
    ...
    \subsection{Solidity IDEs and Debuggers}
        ...
        By contrast, \texttt{SolQDebug} targets pre-deployment authoring, accepts partial fragments
    ...
    \subsection{Solidity Vulnerability Detection and Verification}
        ...
        for smart contracts. However, they target complete programs and focus on vulnerability detection rather than providing edit-time feedback. In contrast, \texttt{SolQDebug}
    ...
    \subsection{Solidity-Specific Abstract Interpretation Frameworks}
        ...
        \texttt{SolQDebug} builds upon these AI frameworks but targets interactive debugging
        rather than invariant generation or information flow analysis. Unlike these approaches that analyze complete contracts, \texttt{SolQDebug} incrementally updates
    ...
    \subsection{Interactive Abstract Interpretation for Traditional Languages}
        ...
        during editing \citep{daig, ds, iac, iaj, fap}. While these systems share \texttt{SolQDebug}'s
        ...
        in traditional languages, \texttt{SolQDebug} represents the first application of this paradigm
...
\section{Conclusion}
    ...
    Additionally, we plan to explore bug detection in the debugging context
    by extending SolQDebug.
    This direction aligns with developer preferences for development-integrated tools
    that provide sound guarantees~\citep{defisecurity}.
...
This appendix provides the complete grammar specification for \texttt{SolQDebug}'s interactive parser.
...
This appendix provides the complete CFG construction patterns for \texttt{SolQDebug}'s incremental CFG builder.
...
\end{tikzpicture}
\caption{Return statement insertion. The \texttt{return} node is rewired to the function's unique \texttt{return exit}}
...
\end{tikzpicture}
\caption{Require statement insertion. The builder creates a \texttt{condition node} with true edge to a node and false edge to the \texttt{error exit}}
...
This appendix presents the abstract domain definitions and formal semantics used by \texttt{SolQDebug}'s abstract interpreter. The framework is based on interval analysis for numeric types, set domains for addresses, and lazy materialization for composite data structures.
