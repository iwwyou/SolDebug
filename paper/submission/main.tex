...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Apply adaptive widening to output (Lines 15--19): use widening operator to combine $out[n]$ with $\hat{o}_{\text{raw}}$ if visit count exceeds $\tau$ at fixpoint evaluation nodes (Line 16), otherwise use join (Line 17).
    \item Check for convergence (Lines 20--21): if output unchanged, skip propagation.
    \item Update $out[n]$ (Line 22); propagate to successors by joining pruned output into their inputs and enqueue (Lines 23--26).
  \end{itemize}
...
\begin{itemize}
  \item \textbf{Syntax validation (Lines 4--7):} Remove lines $[L_s, L_e]$ from $C$ to produce $C'$,
        then validate $C'$ using the Solidity compiler. If $C'$ is invalid, the deletion is rejected.
  \item \textbf{Boundary lookup (Line 8):} \texttt{FindBoundary} retrieves the first and last CFG nodes mapped to the deletion range
        via the line-to-node index $M$, then returns their unique predecessor and successor
        outside the range.
  \item \textbf{Edge removal and reconnection (Lines 9--11):} Detach the edges connecting the deleted region to the surrounding CFG,
  ...
  \item \textbf{Index shift and reinterpretation (Lines 15--16):} Shift line-to-node mappings for all lines after $L_e$ backward by the number of deleted lines, then invoke Algorithm~\ref{alg:reinterpret} with $succ$ as the seed node to propagate updated abstract states.
\end{itemize}
