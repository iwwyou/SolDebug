...
\section{Introduction}\label{sec1}
    ...
    development cycle---not just at deployment. Large language models (LLMs) such as ChatGPT~\citep{gpt} or
    Llama~\citep{llama} can assist with code generation but offer no guarantees of correctness.
    ...
    Foundry Forge~\citep{forge} replicate this costly pipeline, providing no live feedback during edits. A prior
    study found that 88.8\% of Solidity developers described debugging as painful, and 69\%
    attributed this to the absence of interactive, source-level tooling~\citep{interview}. Despite
    ...
    abstract interpretation (AI). Rather than replacing runtime debuggers, it complements them by
    ...
    simulate manually. To support this, \textsc{SolQDebug} applies interval-based AI, which
    generalizes over symbolic inputs, exposes edge-case behaviors, and
    ...
    Second, it performs AI seeded by inline annotations. These annotations,
    ...
    We evaluate \textsc{SolQDebug} on real-world functions from DAppSCAN~\citep{dappscan}, demonstrating
    ...
    \begin{itemize}
      ...
      \item We design an interactive parser with seven specialized entry rules and a dynamic control-flow graph (CFG) engine that supports live structural updates and syntactic recovery during incremental editing.
      ...
      \item We evaluate \textsc{SolQDebug} on 30 real-world functions from DAppSCAN, demonstrating 350$\times$ median speedup over Remix IDE, and analyze how annotation structure impacts precision in multiplication-heavy arithmetic and loop convergence patterns.
    \end{itemize}
...
\end{figure*}
    ...
    Each parsed statement is inserted into a dynamic control-flow graph (CFG), and AI
...
(arrays, mappings, structs). The complete formal semantics (Tables~\ref{tab:conc-denot} and~\ref{tab:abs-denot}) are in Appendix~\ref{app:semantics}.
