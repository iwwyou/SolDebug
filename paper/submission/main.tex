...
edit-time debugging assistant for Solidity that focuses on a single contract/transaction context, providing abstract value summaries with millisecond feedback directly on source code. Developers specify symbolic interval inputs through annotations and compare them
...
from DAppSCAN, achieving millisecond-scale feedback during code editing.
...
\section{Introduction}\label{sec1}
    ...
    This paper presented \textsc{SolQDebug}, an edit-time debugging assistant for Solidity that focused on a single contract/transaction context, powered by abstract interpretation (AI) with interval domain.
    Unlike conventional step-through debuggers that require compilation and deployment, \textsc{SolQDebug} provided abstract value summaries as developers type, enabling exploration of function behavior at the statement level.
    ...
    \begin{itemize}
      ...
      \item \textsc{SolQDebug} was evaluated on 30 real-world functions from DAppSCAN, achieving millisecond-scale feedback latency (median 0.15s). The evaluation demonstrated that overlapping annotation patterns improve precision in common smart contract patterns and provided guidance on annotation design.
    \end{itemize}
...
\end{figure}
    ...
    \subsection{Solidity Execution and Debugging Workflow}
        Unlike traditional programs, Solidity contracts operate within a blockchain
        environment that imposes strict execution constraints. The execution model is
        strictly state-based, where state variables persist across transactions and
        modifications become permanent once confirmed. To debug Solidity code, developers
        must follow a multi-stage workflow as described in Steps 1--4.

        \smallskip
        \noindent\textbf{Step 1: Compile.}
        Figure~\ref{fig:step-compile} shows the compilation step in Remix IDE,
        where the contract source code is compiled into EVM bytecode.
...
\FloatBarrier
        ...
        Deployment is a one-time transaction that stores compiled bytecode on-chain
        and invokes the constructor. Once deployed, the bytecode becomes immutable.

\begin{figure}[!h]
  \centering
  \includegraphics[width=0.9\columnwidth,height=0.4\textheight,keepaspectratio]{Fig3.pdf}
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=1.05\columnwidth, keepaspectratio]{Fig5.pdf}
  \caption{Traditional Solidity debugging workflow: Bytecode-level debugging}
  \label{fig:step-debug}
\end{figure}
\FloatBarrier

        This state-based execution model introduces a critical constraint: changes to contract
        state cannot be undone automatically and require costly external actions such as
        redeployment or manual state reconstruction. This workflow interacts with these
        execution model constraints, introducing significant challenges in the debugging process.
    ...
    \textsc{SolQDebug} addresses the two root causes of Solidity's debugging inefficiency through an integrated design.
    Rather than independent optimizations, the two components jointly constitute the interactive debugging paradigm: incremental analysis provides
    line-by-line responsiveness during editing, while annotations enable input specification---a prerequisite for any debugging activity that observes how inputs affect program behavior.
...
\end{table}
    ...
    annotated symbolic interval inputs as starting points.
...
\begin{itemize}
  ...
  \item \texttt{basic node}: Holds exactly one statement (e.g., a variable declaration, an assignment, or a function call).
...
\texttt{ContractCFG} sequences state variable initializations, the constructor, and function
...
At the function level, each \texttt{FunctionCFG} represents control flow starting from a
function's entry point. When a function declares modifiers, the function body is spliced
into the modifier's control flow at the placeholder (\texttt{\_}) position. When a function invokes another function, the callee's \texttt{FunctionCFG}
...
at the \texttt{current node}, the insertion point for new statements. The following statement
...
\begin{itemize}
  \item \textbf{Simple statements} are spliced between the current node and its successors, creating a single
        \texttt{basic node}. This includes variable declarations, assignments, function calls, and expression statements.
  ...
  \item \textbf{If statement} creates a \texttt{condition node}, true/false \texttt{basic nodes}, and an
        \texttt{if join}. The environment propagates from the current node to the condition, then branches
  ...
  \item \textbf{Else-if clause} replaces the preceding false branch with a new \texttt{condition node}
        and its own join, which then connects to the outer \texttt{if join}. The environment flows through
  ...
  \item \textbf{Else clause} replaces the false branch node of the preceding \texttt{if}/\texttt{else if}
        with a new else node, which connects to the \texttt{if join}. The environment is refined by the
        negated condition and merges at the join (Figure~\ref{fig:new-else}).

  \item \textbf{While loop} creates a \texttt{fixpoint evaluation node} $\phi$, a \texttt{condition node},
        a loop body node, and a \texttt{loop exit node}. The body connects back to $\phi$ for fixpoint iteration.
        The environment propagates through $\phi$ to the condition, then branches to the body or exit with the
        environment refined by the loop condition, with the body flowing back to $\phi$ until convergence
        (Figure~\ref{fig:new-while}).

  \item \textbf{Break statement} is inserted in the loop body with its outgoing edge redirected to the
        \texttt{loop exit node} (Figure~\ref{fig:new-break}).
  ...
  \item \textbf{Return statement} is inserted with its edge rewired to the function's unique \texttt{return exit node},
        detaching original successors. The environment flows directly to the return exit node (Figure~\ref{fig:new-return}).

  \item \textbf{Require statement} creates a \texttt{condition node} with the true edge connecting to a
        continuation node and the false edge pointing to the \texttt{error exit node}. The environment propagates
        with refinement based on the predicate to the continuation or error exit node (Figure~\ref{fig:new-require}).
\end{itemize}
...
\begin{itemize}
  \item \textbf{Find post node (Line 4):} \texttt{FindPostNode}(\(M, L\)) retrieves the first node after line \(L\) from the line-to-node index.
  \item \textbf{Loop-exit/join case (Lines 5--11):} If the post node \(s\) is a loop-exit or join node, we search backward using \texttt{FindPreviousNode}(\(M, L\)) to find the previous node. This returns a condition node if present (loop condition node or \texttt{if}), otherwise the last node before \(L\).
    \begin{itemize}
...
\Function{IncrementalInterpretation}{$G, S$}
  ...
  \While{$wl\neq\langle\rangle$}
    ...
    \State $\hat{\sigma}_{\mathrm{in}}\gets \bigsqcup_{p\in\texttt{Pred}(G, n)} \texttt{RefineByCondition}(p,n)$
...
\begin{itemize}
  ...
  \item \textbf{Incoming environment computation (Lines 11--12):} For each predecessor $p$, \texttt{RefineByCondition} examines the edge from $p$ to $n$. If $p$ is a condition node, it prunes the predecessor's output environment based on the branch direction (true or false), removing infeasible paths. For non-condition predecessors, no pruning is applied. At join nodes with two predecessors, the resulting states are merged via $\sqcup$. Single-predecessor nodes receive the (possibly pruned) state directly as $\hat{\sigma}_{\mathrm{in}}$.
        For \texttt{require} and \texttt{assert}, which create condition nodes, the true branch receives the pruned environment while the false branch leads to the error exit node. For \texttt{revert}, control transfers directly to the error exit node without creating a condition node. Since the error exit node is a sink node, the \texttt{IsSink} check filters it out and propagation terminates along exceptional paths.
...
\Function{LoopFixpoint}{$G, h$}
  ...
  \While{$wl\neq\langle\rangle$}
    ...
    \If{$in[n] = \bot$} \Comment{compute input if not initialized}
       \State $in[n] \gets \bigsqcup_{p\in\texttt{Pred}(G, n)\cap L} \texttt{RefineByCondition}(p,n)$
    \EndIf
  ...
  \State $\Env(exit) \gets \bigsqcup_{p\in\texttt{Pred}(G, exit)} \texttt{RefineByCondition}(p, exit)$ \Comment{join with false-branch pruning}
  \State \Return $exit$
\EndFunction
\end{algorithmic}
\end{algorithm}
...
\end{itemize}
yielding 120 candidate contracts. Since \textsc{SolQDebug} focuses on single-contract, single-transaction analysis, functions were systematically filtered based on four exclusion criteria, applied in the following priority order:

\begin{enumerate}
  \item \textbf{External dependencies}---functions involving cross-contract interactions were excluded, including: external contract calls (e.g., \texttt{IERC20(token).transfer()}), library function calls via Solidity's \texttt{using} directive, low-level calls (\texttt{.call()}, \texttt{.delegatecall()}, \texttt{.staticcall()}), parent contract invocations via \texttt{super}, and functions using inherited modifiers not defined in the analyzed contract (e.g., \texttt{onlyOwner} from OpenZeppelin's \texttt{Ownable}).
  ...
  \item \textbf{Data builtin functions}---functions using Solidity's built-in data transformation operations were excluded, including: cryptographic hash functions (\texttt{keccak256}, \texttt{sha256}, \texttt{ecrecover}) and ABI encoding/decoding functions (\texttt{abi.encode}, \texttt{abi.decode}, \texttt{abi.encodePacked}). These operations are opaque to source-level analysis and their internal computations cannot be traced step-by-step.

  \item \textbf{Trivial functions}---functions with minimal logic that provide no meaningful debugging targets were excluded, including: single-statement returns, assignments, or function calls; functions containing only \texttt{require}/\texttt{assert} guards; and functions with empty bodies.
\end{enumerate}

\noindent The filtering criteria were applied hierarchically: if a function matched an earlier criterion, subsequent criteria were not evaluated. This ordering reflects the nature of each limitation: external dependencies represent fundamental scope boundaries (cross-contract analysis is not supported), while inline assembly and data builtin functions are internal features that could potentially be supported in future versions. Trivial functions, though within scope, simply lack sufficient complexity to benefit from debugging analysis. This hierarchical approach ensures each function is counted in exactly one exclusion category.

\noindent Table~\ref{tab:filtering-stats} summarizes the filtering results across all 120 contracts. Of the 1,529 functions extracted, external dependencies account for the largest exclusion category (70.8\%), reflecting the heavily modular nature of modern DeFi contracts that extensively rely on OpenZeppelin libraries and cross-contract interactions. Trivial functions represent the second largest category (18.4\%), consisting of simple getters, setters, and single-statement operations. After filtering, 142 candidate functions (9.3\%) remain suitable for debugging analysis.
...
\begin{tabular}{lrr}
\toprule
\textbf{Category} & \textbf{Functions} & \textbf{\%} \\
\midrule
...
\quad 3. Data builtin functions & 21 & 1.4 \\
...
RQ1 evaluates the reduction in debugging workflow latency---specifically, the time required for developers to understand program behavior under specific input ranges. While various abstract interpretation techniques have been proposed for other programming languages, such as demanded summarization-based approaches for incremental reanalysis~\citep{daig}, these methods focus on optimizing static analysis pipelines rather than supporting interactive debugging tasks. In contrast, Remix IDE provides debugging functionality that allows developers to trace execution flows for user-specified inputs, which aligns with the task that \textsc{SolQDebug} aims to support. Therefore, we select Remix as our baseline as it is the only existing Solidity tool capable of performing the same debugging task.

\smallskip
\noindent\textbf{Measurement.}
To evaluate responsiveness, debugging latency was measured, defined as the time
...
\begin{figure}[t]
  ...
  \caption{Debugging workflow latency comparison between \textsc{SolQDebug} and Remix across varying
  ByteOp counts and test-case widths \(\Delta\) (5-run mean values). \textsc{SolQDebug} maintains consistent
...
120 total measurements for \textsc{SolQDebug}. To ensure statistical validity, each measurement was repeated 5 times, and mean latencies were computed for analysis.
Figure~\ref{fig:rq1-responsiveness} illustrates the latency comparison across these dimensions using 5-run mean values.

To assess statistical significance, we employed the Wilcoxon signed-rank test, a non-parametric method for comparing paired samples. The p-value represents the probability of observing results at least as extreme as the measured data under the null hypothesis~\citep{pvalue}. A p-value below 0.05 indicates that the observed difference is statistically significant.

For Remix, the debugging workflow latency ranged from 25.6 to 128.7 seconds (mean: 54.62s, median: 51.68s), reflecting
...
In contrast, \textsc{SolQDebug} completed analysis in 0.02--1.88 seconds (mean: 0.24s, median: 0.09s)
across all 30 contracts. \textsc{SolQDebug}'s latency remains nearly constant
regardless of test-case width, as AI analyzes all input combinations symbolically in a single pass.
Across 30 smart contracts, \textsc{SolQDebug} demonstrated significantly lower debugging workflow latency than Remix (Wilcoxon signed-rank test, $n=30$, $p = 9.31 \times 10^{-10}$).
This comparison reflects debugging workflow time, measuring how quickly developers gain insights into function behavior rather than comparing equivalent computational tasks. Remix requires compilation, deployment, and concrete execution tracing, while \textsc{SolQDebug} provides immediate statement-level abstract summaries during editing.

\medskip
\noindent\fbox{%
\begin{minipage}{0.96\linewidth}
\textbf{Answer to RQ1:}
\textsc{SolQDebug} achieves sub-second feedback (median 0.09s), providing significantly lower debugging workflow latency compared to Remix's compile-deploy-debug cycle (median 51.68s) with statistical significance (Wilcoxon test, $p < 0.001$). The two tools take fundamentally different approaches: \textsc{SolQDebug} generates statement-level abstract summaries over symbolic inputs, while Remix traces concrete execution paths. Unlike Remix, whose latency scales linearly with input space size, \textsc{SolQDebug}'s performance through abstract interpretation does not depend on test-case width.
\end{minipage}%
}
...
To mitigate this threat, we isolated the pure debugging time
and measured it using the JavaScript \texttt{performance.now()} API within the browser,
minimizing the impact of automation overhead.
...
& Index-related arithmetic & Arithmetic where operands relate to loop index (e.g., \texttt{i}, \texttt{arr[i]}) can converge with proper annotation. & Bounds-aware analysis tracks index-correlated computations. \\[0.5em]
...
For loop constructs, precision depends on: (1) explicit initialization of local variables before the loop, (2) proper annotation of loop bounds, and (3) whether arithmetic operations inside the loop involve operands related to the loop index. Operations on index-unrelated variables cause unavoidable widening, and such widened variables should be isolated from subsequent computations when precise analysis is required.
    ...
    The current implementation does not support inline assembly blocks. Inline assembly allows developers to write raw EVM opcodes within Solidity, bypassing the type system and enabling low-level optimizations. Since assembly instructions operate directly on stack and memory without Solidity's semantic abstractions, the current CFG construction and abstract interpretation cannot reason about their effects. As a minimal viable step, assembly blocks could be treated conservatively by assigning TOP to all variables potentially modified within the block. A more refined approach would pattern-match common assembly idioms (e.g., \texttt{mload}/\texttt{mstore} for memory operations, \texttt{sload}/\texttt{sstore} for storage) and translate them into equivalent abstract operations.

    \smallskip
    \noindent\textbf{Contract Interaction.}
    Inter-contract calls, inheritance hierarchies, and multi-transaction workflows are currently out of scope. For multi-contract support, a summary-based approach would analyze callee contracts separately and represent their effects as function summaries (input-output interval relations) at call sites. This avoids full inlining while preserving soundness. Multi-transaction analysis would additionally require serializing abstract states between transaction boundaries.

    \smallskip
    \noindent\textbf{Bug Detection.}
    The current system functions as a debugging assistant that displays variable values and execution paths. A natural extension is proactive bug detection: once interval information is available, a validation engine can check for common vulnerability patterns. The target bugs are logic errors, particularly numerical defects such as arithmetic overflow, precision loss, and accounting errors~\citep{numscout, gptscan, accountingerrors}. This evolution from ``showing values'' to ``detecting problems'' aligns with the debugging assistant paradigm and with developer preferences for development-integrated tools with sound guarantees~\citep{defisecurity}.

    \smallskip
    \noindent\textbf{User Experience Validation.}
    A formal developer study has not yet been conducted.
    Since \textsc{SolQDebug} currently operates without full GUI/IDE integration,
    a developer study at this stage would not accurately reflect usability in complete development settings.
    Meaningful usability evaluation requires additional HCI/GUI research to provide intuitive interfaces for annotation input, result visualization, and seamless editor integration.
    We plan to conduct formal usability studies once the above extensions are implemented alongside comprehensive IDE integration.
...
\section{Conclusion}
    ...
    Evaluation on 30 real-world DAppSCAN functions demonstrated millisecond-scale feedback latency. Our analysis showed that annotation-guided interpretation
...
\bibitem[Wasserstein and Lazar(2016)]{pvalue}
...
\end{tikzpicture}
\caption{Require statement insertion. The builder creates a \texttt{condition node} with true edge to a node and false edge to the \texttt{error exit node}}
