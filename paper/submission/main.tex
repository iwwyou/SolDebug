...
\section{Introduction}\label{sec1}
    ...
    with millisecond-scale overhead. Debug annotations allow developers to specify symbolic interval inputs
    ...
    First, it provides incremental analysis through interactive parsing and incremental control-flow graph (CFG) construction.
    ...
    Second, it enables annotation-guided exploration. Developers specify symbolic interval inputs
    ...
    \begin{itemize}
      \item We identify the main barriers to interactive Solidity debugging: latency from compilation, deployment, and transaction setup, and Ethereum Virtual Machine (EVM) constraints that prevent re-execution.
      \item We design an interactive parser with seven specialized entry rules and an incremental CFG engine that supports live structural updates and syntactic recovery during incremental editing.
      \item We introduce an annotation-guided abstract interpreter with adaptive widening thresholds that uses developer-specified symbolic interval inputs to achieve precise loop analysis while maintaining termination guarantees.
...
contract Example {
    ...
    function burn(uint256 amount) public onlyOwner {
        ...
        Listing~\ref{lst:grammar-min} illustrates the key structural elements of a Solidity smart
        contract. At the top level, Solidity programs may declare contracts, interfaces, and
        libraries. A contract is composed of variables and functions.
    ...
    \subsection{Solidity Execution and Debugging Workflow}
        Unlike traditional programs that can be executed locally and repeatedly, Solidity contracts
        operate within a blockchain environment that imposes strict execution constraints. To execute
        a Solidity contract on the blockchain, it must first be deployed. Deployment occurs through a
        one-time transaction that stores the compiled bytecode on-chain and invokes the constructor
        exactly once. This process is fundamentally different from conventional software deployment
        because the bytecode becomes immutable once recorded on the blockchain.

        After deployment, all subsequent interactions are message-call transactions in which the
        caller specifies a public function along with encoded calldata. Once the transaction is mined
        into a block, the EVM jumps to the designated entry point and executes the corresponding
        function sequentially. The execution model is strictly state-based. State variables persist
        across transactions, forming a global storage that is shared by all function calls. Each
        transaction can read from and write to this persistent state, and these modifications become
        permanent once the transaction is confirmed.

        This state-based execution model introduces a critical constraint. Side effects are
        irreversible without external intervention. Once a function executes and modifies the
        contract state, returning to the previous state requires either redeploying the entire
        contract or manually reconstructing the initial conditions through additional transactions.
        Redeployment discards all accumulated state and resets the contract to its initial
        configuration. Manual state reconstruction requires developers to identify which state
        variables were modified and issue preparatory transactions to restore their original values.

        To debug Solidity code, developers follow a multi-stage workflow illustrated in
        Figure~\ref{fig:legacy-debug-grid}. Before invoking the target function, the contract must be
        compiled and its state must be manually initialized through setup transactions to satisfy any
        ...
        above, creating fundamental bottlenecks in the debugging process.
...
\end{figure*}
    ...
    \subsection{Root Causes of the Solidity-Debugging Bottleneck}
        ...
        As described in the previous subsection, the EVM's state-based execution model makes side
        effects irreversible. This constraint fundamentally conflicts with iterative debugging, which
        requires repeatedly re-executing the same function under different conditions.
    ...
    pair of complementary techniques. The first technique targets external latency from blockchain
    round trips, while the second addresses internal opacity due to storage-based semantics.

    \smallskip
    \noindent\textbf{(1) Removing the multi-stage workflow.}
    ...
    the Solidity Editor. To support live editing, we extend the Solidity grammar with interactive
    ...
    the edit point. We use abstract interpretation with interval domain because it provides three key
    ...
    \smallskip
    \noindent\textbf{(2) Enabling repeated execution via batch annotations.}
    ...
    top of the function to define initial abstract values. These values are injected before analysis
...
\section{The design of SolQDebug}
    \textsc{SolQDebug} processes code incrementally as developers edit it, building up
    ...
    corresponding construct is stored in a node that is inserted at a semantically valid
    location, determined from the surrounding context and the existing control flow. Third,
    when batch debug annotations are present, the system re-analyzes the function using the
    annotated initial values.
    The following subsections detail the architecture and core mechanisms that enable this incremental analysis.

    \subsection{System Architecture}
        Figure~\ref{fig:solqdebug-arch} illustrates the overall architecture of \textsc{SolQDebug}.
        \textsc{SolQDebug} accepts either single Solidity statements or batch debug annotations as input.
        These inputs are processed through two main modules:

        \smallskip
        \noindent\textbf{(1) Parsing Module.}
        Each input passes through the \textsc{Context Analyzer}, which extracts the
        surrounding source context needed to parse the partial statement or annotation. The
        \textsc{Interactive Parser}, built on ANTLR~\citep{antlr}, applies an extended grammar that adds
        ...
        added, creating corresponding nodes for each statement and rewiring control edges to reflect
        the updated program structure. The \textsc{Abstract Interpreter} analyzes the
        updated CFG, reusing previous results and computing abstract values only for affected program
        points. The \textsc{Snapshot Manager} ensures
        ...
        \smallskip
        \noindent\textbf{(3) Line-Level Output.}
        Following analysis, \textsc{SolQDebug} produces a statement-level summary showing the computed intervals
    ...
    This example demonstrates two key analysis modes: incremental source code analysis (\S{}3.2.1)
...
\begin{itemize}
  ...
  \item \textbf{Statement fragment:} Ends with semicolons
...
We focus on Step~9 (\texttt{totalSupply -= delta;}), which illustrates how \textsc{SolQDebug} handles CFG
insertion after a conditional branch merge. When Step~9 arrives, it processes the statement
as follows:
...
\begin{enumerate}
  ...
  \item The \textsc{Abstract Interpreter} performs worklist-based reinterpretation, propagating the updated
        environment from the newly inserted node to all forward-reachable nodes in the CFG.
\end{enumerate}
...
obtain statement-level results in a single run.
...
Listing~\ref{lst:grammar-batch} shows the \texttt{burn} function with batch annotations.
        ...
        \texttt{@LocalVar} for local variables) and assigns an symbolic interval input, supporting both
...
In this example, we annotate \texttt{balances[msg.sender]} with the interval \([100,200]\)
        ...
        When a batch annotation block is encountered, \textsc{SolQDebug} follows this pipeline:
...
\begin{enumerate}
  ...
  \item \textbf{Restore previous state.} After analysis, the unannotated memory is restored.
\end{enumerate}
...
\textsc{SolQDebug} internally validates that continuation rules have their required antecedent structure,
preventing invalid fragments (e.g., a standalone \texttt{else}) from being parsed. This ensures
that even though the parser accepts partial code, the resulting program structure remains
well-formed at each step.

For concreteness, we refer to the burn function in Table~\ref{tab:input_code}.
When the developer types the function definition, it is parsed by \texttt{interactiveSourceUnit},
...
which handles both statement fragments and block fragments (see Appendix~\ref{appendix:interactive-grammar}).
...
\begin{itemize}
  \item \textbf{\texttt{GlobalVar}} assigns values to global variables such as msg.sender or block.timestamp
  \item \textbf{\texttt{StateVar}} assigns values to contract state variables, supporting nested access patterns like balances[msg.sender] or user.balance
  \item \textbf{\texttt{LocalVar}} assigns values to function parameters
\end{itemize}

The primary value format is integer intervals (e.g., \texttt{[100,200]}).
The parser validates type compatibility at parse time, reporting errors
if annotated values are incompatible with declared types.
...
We then explain how individual statements are incrementally spliced into the function-level CFG (\S3.4.2),
...
and function definitions receive the environment that results from constructor execution.
...
into the modifier's control flow at the placeholder (\texttt{\_}) position, which may
appear before, after, or around the function's own statements depending on the modifier's
structure. When a function invokes another function, the callee's \textsc{FunctionCFG}
...
at the \textsc{current node}, the point where new statements are added. Different statement
types create different CFG structures, and the abstract environment propagates through the
newly created nodes according to each statement's semantics:
...
\begin{itemize}
  ...
  \item \textbf{Loop-exit/join case (Lines 5--11):} If the post node \(s\) is a loop-exit or join node, we search backward using \textsc{FindPreviousNode}(\(M, L\)) to find the previous node. This returns a condition node if present (loop condition node or \texttt{if}), otherwise the last node before \(L\). If it is a condition node, we return its TRUE branch to place the new statement inside the construct (Line 8); otherwise, we return the node itself (Line 10).
  \item \textbf{Basic post node (Lines 12--19):} Otherwise, \(s\) is a basic statement node (Line 12). We retrieve its CFG predecessors and verify there is exactly one (Lines 13--15), which must hold by construction since conditionals are created with their join nodes and loops are created with their exit nodes. Any other count indicates a malformed CFG (Lines 16--18).
\end{itemize}
...
For debug annotations, \textsc{SolQDebug} performs full interpretation from the function entry node, ensuring all inspection points receive complete abstract states.
...
\begin{itemize}
  ...
  \item \textbf{Seed node initialization (Lines 5--9):} Enqueue all seed nodes $s \in S$, which mark insertion points for incremental edits or function entry for batch annotations, filtering out sink nodes (exit, error, return) that have no successors to propagate to.
...
Algorithm~\ref{alg:fixpoint} computes loop fixpoints using annotation-aware adaptive widening.
...
\begin{itemize}
  ...
  \item \textbf{\textsc{EstimateIterations} (Line 5):} Evaluate both operands of the loop condition in the pre-loop environment $start$. For comparison operators such as $<, \le, >, \ge$, compute $\tau$ as the difference between operand bounds. For example, given a condition \texttt{i < array.length} where \texttt{i = [0,0]} and \texttt{array.length = [10,10]}, the function computes $\tau = 10 - 0 = 10$. This requires that both operands evaluate to concrete intervals where if either operand is a complex expression, it must reduce to a constant interval through annotation-provided values. When annotations constrain values to precise intervals, this yields tight thresholds. Otherwise, a default threshold of 1 is used, allowing one iteration to initialize node environments before applying widening in subsequent iterations.
  ...
  \item \textbf{Widening phase (Lines 10--26):} Perform fixpoint iteration with worklist-based propagation (Lines 9--27). For each node, dequeue and increment visit count (Line 10), compute its incoming environment if not yet initialized (Lines 11--13). Apply the transfer function to compute raw output (Line 14). Lines 15--19 implement adaptive widening where at the fixpoint evaluation node, if the visit count exceeds $\tau$, the algorithm applies the widening operator (Line 16), and otherwise uses the join operator (Line 18). Lines 20--22 check whether the output has changed from the previously stored output. If unchanged, this node has reached its fixpoint with the environment stabilized, so the algorithm continues to the next worklist item without propagating to successors (Line 21). Otherwise the algorithm stores the new output (Line 23) and propagates the changed result to successors (Lines 24--26).
...
\begin{itemize}
  ...
  \item \textbf{RQ2 -- Impact of Annotation Patterns on Precision}:\;%
        In complex arithmetic operations involving multiplication and division, how does the structure
...
We evaluate \textsc{SolQDebug} on single-contract, single-transaction functions
to validate our contributions of incremental CFG construction and
...
functions that contain only assignments or return statements. From the
...
For example, providing annotations \texttt{// @StateVar actionBuilders = arrayAddress[1,2,3];} and \texttt{// @LocalVar index = [3,3];} yields \texttt{additionalCount = 3 - 3 + 1 = 1}, allowing the analyzer to set $\tau = 1$ and achieve precise convergence with the final result \texttt{i = [0,1]}.
...
For example, providing annotations \texttt{// @StateVar depositsOf[\_account][0].amount = [100,200];}, \texttt{// @StateVar depositsOf[\_account][1].amount = [200,300];}, and \texttt{// @StateVar depositsOf[\_account][2].amount = [400,500];} yields a loop bound of 3, setting $\tau = 3$.
The analyzer correctly infers \texttt{i = [0,3]} after the loop.
...
For example, providing annotations \texttt{// @StateVar \_tokens = array [1,2,3];} and \texttt{// @LocalVar tokenId = [4,4];} yields \texttt{tokenCount = 3}, setting $\tau = 3$.
The analyzer correctly infers \texttt{tokenIndex = [0,3]} after the loop.
However, inside the loop, \texttt{resultIndex} increments conditionally (\texttt{if (tokenItemId != tokenId) resultIndex++}) based on array element comparisons.
...
\subsection{Why use Abstract Interpretation with Interval Domain for Debugging}
    ...
    occurs during code editing, before deployment. Developers vary symbolic interval inputs
    and immediately observe program behaviors at the source
    ...
    We use AI with interval domain because it uniquely satisfies these debugging requirements,
    offering three key properties:
    \begin{itemize}
      \item \textbf{Termination.} AI enforces convergence through widening at loops and joins at control-flow merges, avoiding path explosion in symbolic execution.
      \item \textbf{Explainability.} With interval domains, the mapping from inputs to outputs is explicit as ranges, enabling developers to trace dataflow effects and reason about behavior at the statement level.
      \item \textbf{Responsiveness.} Unlike relational domains (e.g., zones, polyhedra) that track variable correlations, intervals require only basic arithmetic on bounds, enabling millisecond-scale updates that align with the edit cycle.
    \end{itemize}

    Although AI's precision is inherently conservative, we address this limitation while preserving
    edit-time responsiveness by providing developers with techniques to control precision
    through annotation design. Our evaluation shows that such techniques can reduce
    imprecision in practice (cf.\ RQ2, RQ3).
    \vspace{0.25em}
    ...
    responsiveness, developers encountering such patterns may require more expressive abstract
    domains such as relational domains for more precise analysis.
...
\section{Related Works}
    ...
    \subsection{Solidity IDEs and Debuggers}
        ...
        is a Node.js-based framework that couples the Solidity compiler with an Ethereum runtime. Its Hardhat
        Debug plug-in provides step-by-step debugging of transaction execution within Visual Studio Code. Foundry Forge~\citep{forge} is a command-line toolchain oriented toward fast,
    ...
    \subsection{Solidity Vulnerability Detection and Verification}
        ...
        for smart contracts. However, they are designed to find security issues rather than
        to support interactive debugging during development. In contrast, \textsc{SolQDebug}
        is a debugging tool that helps developers understand code behavior interactively
        during editing.
    ...
    \subsection{Solidity-Specific Abstract Interpretation Frameworks}
        ...
        rather than invariant generation or information flow analysis. It incrementally updates
        the control-flow graph and abstract state during editing. Developers specify symbolic
        symbolic interval inputs through annotations, which guide interval analysis. This approach provides
        edit-time feedback without requiring complete contracts.
    ...
    \subsection{Interactive Abstract Interpretation for Traditional Languages}
        ...
        to Solidity. More fundamentally, it introduces an annotation-guided approach where developers
        specify symbolic interval inputs directly in source code, and the system uses these annotations
        to guide interval analysis. This design bridges the gap between Solidity development and the
        interactive tooling common in traditional programming environments.

\section{Conclusion}
    This paper presents SolQDebug, the first interactive source-level debugger for Solidity
    that eliminates the traditional compile-deploy-debug cycle. Through interactive parsing,
    dynamic control-flow analysis, and annotation-guided abstract interpretation, SolQDebug
    provides millisecond-scale feedback directly within the editor.

    Evaluation on 30 real-world DAppSCAN functions demonstrates a 350$\times$ median speedup
    over Remix IDE (0.15s vs. 53.0s). Our analysis shows that annotation-guided interpretation
    enables precise analysis across diverse execution paths while maintaining soundness guarantees.
    These results demonstrate that SolQDebug effectively bridges the interactivity gap in
    ...
    conducting user studies to assess practical adoption, and applying EOF-based analysis
