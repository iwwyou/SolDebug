...
\captionsetup{compatibility=false}% Suppress unknown document class warning
...
\orgaddress{\street{145, Anam-ro}, \city{Seongbuk-gu}, \postcode{02841}, \state{Seoul},
...
\abstract{As Solidity becomes the dominant language for blockchain smart contracts,
    ...
    To address this challenge, this paper presents SolQDebug,
    ...
    providing abstract value summaries with millisecond-scale feedback directly on source code.
    ...
    SolQDebug was evaluated on 30 real-world functions from DAppSCAN, achieving millisecond-scale feedback.
    This paper also synthesizes experimental observations on annotation patterns for arithmetic operations and loop constructs into practical guidelines.
    These results demonstrate that SolQDebug serves as an effective edit-time debugging assistant for Solidity development.}
...
\section{Introduction}\label{sec1}
    ...
    This is especially important because once deployed to the blockchain, contracts are immutable and cannot be easily fixed.
    Many developers now rely on large language models (LLMs) such as ChatGPT~\citep{gpt} or Llama~\citep{llama} for code generation assistance.
    ...
    To the best of our knowledge, no existing research or tooling provides interactive feedback for Solidity development, a gap that this paper aims to fill.
    
    This paper presents SolQDebug, an edit-time debugging assistant for Solidity designed for interactive analysis within a single contract/transaction context,
    powered by abstract interpretation (AI) with the interval domain.
    Unlike conventional step-through debuggers that require compilation and deployment,
    SolQDebug provides abstract value summaries as developers type, enabling exploration of function behavior at the statement level.
    To achieve this goal, SolQDebug builds on two core ideas.
    First, it provides incremental analysis through interactive parsing and incremental control flow graph (CFG) construction via line-to-node indexing.
    ...
    Second, it enables annotation-guided abstract interpretation. Developers specify symbolic interval inputs through annotations,
    and the interpreter captures multiple input scenarios over various execution paths in a single analysis.

    To validate these design choices, SolQDebug was evaluated on 30 real-world functions from DAppSCAN~\citep{dappscan}.
    ...
    Additionally, a pilot study confirmed that manual value tracing is demanding, highlighting the need for integrated debugging assistance.
    ...
    \begin{itemize}
      ...
      \item An interactive parser with seven specialized entry rules and line-to-node indexing was designed to enable incremental CFG construction with efficient statement insertion during editing.
      ...
      \item SolQDebug was evaluated on 30 real-world functions from DAppSCAN, achieving millisecond-scale feedback latency. The experimental findings were synthesized into practical guidelines for annotation patterns in arithmetic operations and loop constructs.
    \end{itemize}
\vspace{-1em}
...
\section{Background}\label{sect2}
    \subsection{Structure of a Solidity Smart Contract}
\begin{figure}[!ht]
...
\end{figure}
        ...
        Global variables such as \texttt{msg.sender} (appearing at line 6) and \texttt{block.timestamp} represent read-only EVM metadata provided implicitly by the runtime.
        ...
        Local variables such as \texttt{bal} and \texttt{delta} (lines 10--11) are scoped to a single function call and do not persist beyond execution.

        Functions consist of statements such as assignments, conditionals (\texttt{if}/\texttt{else}), and loops (\texttt{while}/\texttt{for}).
        The \texttt{burn} function (lines 9--20) illustrates this structure.
        ...
        The \texttt{onlyOwner} modifier (lines 5--8) verifies that \texttt{msg.sender} equals \texttt{owner} before executing the function body.
        The placeholder underscore at line 7 marks where the original function body is inserted when the modifier is applied.

    \subsection{Solidity Execution Model and Debugging Workflow}
        Unlike traditional programs, Solidity contracts execute in a state-based environment that imposes a distinctive multi-stage debugging workflow.
        This workflow consists of four stages, as described in Steps 1--4.
...
\FloatBarrier
        ...
        Figure~\ref{fig:step-init} shows how a developer enters parameters and executes the target function for debugging.
...
\FloatBarrier
        ...
        After the target function completes, its behavior is traced step by step at the bytecode level.
...
\FloatBarrier
    ...
    \subsection{Two Sources of Inefficiency in Solidity Debugging}
        ...
        Solidity development requires interaction with an external blockchain node at every stage.
        ...
        Given this workflow overhead, developers often rely on emitted events (via \texttt{emit}) to observe intermediate values.
        However, such instrumentation provides only runtime snapshots of a single execution.
        Moreover, changing what is being logged typically requires recompilation and redeployment, compounding latency and disrupting the iterative debugging process.
        ...
        with 69\% attributing their frustration to the lack of interactive, state-aware tooling.

    \subsection{Proposed Approach Overview}

        SolQDebug addresses these two inefficiencies through an integrated design.
        Rather than serving as independent optimizations, the two components jointly constitute an edit-time debugging assistant.
        ...
        SolQDebug eliminates this multi-stage pipeline by performing both parsing and abstract interpretation directly inside the Solidity editor.
        ...
        It guarantees termination through widening and produces interpretable results in the form of variable ranges.
        ...
        Both approaches disrupt iterative debugging. To address this, SolQDebug introduces batch annotations, enabling developers to vary inputs or contract state to observe control-flow outcomes.
...
\begin{figure*}[t]
  ...
  \vspace{0.3cm}
  \caption{SolQDebug architecture}
  \label{fig:solqdebug-arch}
\end{figure*}

\section{The design of SolQDebug}
    SolQDebug is designed to analyze single-contract, single-transaction Solidity functions
    through incremental statement-by-statement processing. As developers write code,
    the system maintains an evolving abstract state of the program, enabling
    ...
    and explain the core mechanisms for parsing (\S3.3), incremental analysis (\S3.4), and code deletion and modification (\S3.5).

    \subsection{System Architecture}
        Figure~\ref{fig:solqdebug-arch} illustrates the overall architecture of SolQDebug.
        ...
        validates it using the official Solidity compiler before proceeding to analysis. This ensures
        semantic consistency and rejects malformed input early.
        ...
        Following analysis, SolQDebug produces a summary showing computed intervals at program points
        where values are defined, assigned, returned, or changed through loop iterations. Loop-related changes are identified by comparing pre-loop and post-loop abstract states.
  ...
  \begin{tabularx}{\columnwidth}{@{}c c X@{}}
    ...
    1 & 9--10 &
      \begin{tabular}[t]{@{}l@{}}
        ...
        \}
      \end{tabular} \\
    2 & 10 & uint256 bal = balances[msg.sender]; \\
    3 & 11 & uint256 delta; \\
    4 & 12--13 &
      \begin{tabular}[t]{@{}l@{}}
        if (bal >= amount) \{\\
        \}
      \end{tabular} \\
    5 & 13 & balances[msg.sender] = bal - amount; \\
    6 & 14 & delta = amount; \\
    7 & 16--17 &
      \begin{tabular}[t]{@{}l@{}}
        else \{\\
      ...
      \end{tabular} \\
    8 & 17 & delta = 0; \\
    9 & 19 & totalSupply -= delta; // new input \\
    \bottomrule
  \end{tabularx}
  \rmfamily
\end{table}

    To illustrate how the proposed architecture operates in practice, we present a concrete example using the \texttt{burn} function from Figure~\ref{fig:grammar-min}.
...
As the developer types each fragment, SolQDebug incrementally extends the CFG
and recomputes abstract values only for affected program points.
Figure~\ref{fig:solqdebug-cfg} visualizes the CFG structure after all nine steps have been integrated, highlighting the insertion of Step 9.
...
When Step~9 arrives, SolQDebug processes the statement as follows:
...
This reinterpretation ensures soundness by propagating the effects of each edit to all
...
function burn(uint256 amount) public onlyOwner {
    // @Debugging BEGIN
    // @StateVar balances[msg.sender] = [50,200]
    // @LocalVar amount = [100,150]
    // @Debugging END
    uint256 bal = balances[msg.sender];
    uint256 delta;
    if (bal >= amount) {
...
In this example, we annotate \texttt{balances[msg.sender]} with the interval \([50,200]\)
        and \texttt{amount} with \([100,150]\) to explore how the \texttt{burn} function behaves
...
\begin{enumerate}
  ...
  \item \textbf{Single-pass analysis.} SolQDebug re-analyzes the pre-built CFG with
...
SolQDebug validates that continuation rules have their required antecedent structure,
...
which handles both statement fragments (Steps 2, 3, 5, 6, 8, and 9) and block fragments (Step 4).
...
During incremental CFG construction, the system maintains a control-flow graph that evolves as developers insert new statements.
Rather than rebuilding from scratch, the graph is modified in place.
...
\begin{itemize}
  ...
  \item \texttt{fixpoint evaluation node} (\(\phi\)): The loop join point used for widening and narrowing during fixpoint computation (\S3.4.4).
...
SolQDebug organizes control flow at two levels.
...
When a function declares modifiers, the modifier's control flow wraps the function body, with the placeholder (\texttt{\_}) marking where the function body is inserted.
...
These construction patterns enable SolQDebug to build the CFG incrementally as the developer types each statement, with environment updates propagating only through affected nodes.
...
In contrast, SolQDebug must handle partial code edits that specify only target line numbers.
Since CFG edges encode control flow rather than source positions, source-line information alone is insufficient to determine where an edit belongs.
...
Most statements map to a basic node, except \texttt{require}/\texttt{assert}, which maps to a condition node.
...
\begin{itemize}
  ...
  \item \textbf{Loop-exit/join case (Lines 5--11):} If the post node \(s\) is a loop-exit or join node, search backward using \texttt{FindPreviousNode}(\(M, L\)) to find the previous node. This returns a condition node if one exists (loop condition node or \texttt{if}). Otherwise, it returns the last node before \(L\).
    ...
    \end{itemize}
  \item \textbf{Basic post node (Lines 12--19):} Otherwise, \(s\) is a basic statement node (Line 12). Retrieve and verify the CFG predecessors to be exactly one (Lines 13--15). This invariant holds by construction: a basic statement node always has exactly one predecessor, since branching only occurs at condition nodes and merging only occurs at join/exit nodes. Any other count indicates a malformed CFG (Lines 16--18).
\end{itemize}

        \subsubsection{Abstract Interpretation for Incremental Analysis}\label{sec:impl}
SolQDebug provides rapid feedback on source code edits by propagating updates only along affected CFG paths, avoiding full re-analysis.
...
often causing intervals to expand to $\top$ (the full type range) even when the actual loop bounds are finite.
...
The key innovation is that \texttt{EstimateIterations} analyzes loop conditions to compute an adaptive threshold $\tau$ that defers widening. When debug annotations provide precise interval bounds for condition operands such as array lengths or parameter values, the analyzer raises $\tau$ to delay widening and preserve precision. The fixpoint iteration terminates when all node outputs stabilize, i.e., when outputs equal their previously stored values. Soundness is guaranteed by the monotonicity of the transfer functions and the correctness of the widening operator.

\begin{itemize}
  \item \textbf{Identify loop nodes and pre-loop environment (Line 4):} Collect all nodes within the loop body. Compute $start$ by joining environments from all loop-entry predecessors, specifically those from outside the loop.
  ...
  \item \textbf{Widening phase (Lines 9--26):} Perform fixpoint iteration with worklist-based propagation. For each iteration:
  \begin{itemize}
    \item Dequeue node $n$ and increment visit count (Line 10).
    \item Initialize input environment if unset (Lines 11--13): compute $in[n]$ by joining predecessor outputs $out[p]$, each pruned by the edge condition $p \to n$.
    \item Compute raw output $\hat{o}_{\text{raw}}$ by applying transfer function to current input (Line 14).
    \item Apply adaptive widening to output (Lines 15--18): use widening operator to combine $out[n]$ with $\hat{o}_{\text{raw}}$ if visit count exceeds $\tau$ at fixpoint evaluation nodes (Line 16), otherwise use join (Line 17).
    \item Check for convergence (Lines 19--20): if output unchanged, skip propagation.
    \item Update $out[n]$ (Line 21); propagate to successors by joining pruned output into their inputs and enqueue (Lines 22--25).
  \end{itemize}

  \item \textbf{Narrowing phase and exit node (Lines 27--30):} Apply standard narrowing using descending iteration with narrowing operator at fixpoint evaluation nodes to refine over-approximations from widening (Line 27). Then compute the exit node environment (Lines 28--29) by joining predecessor outputs $out[p]$, each pruned by the edge condition $p \to exit$ (i.e., the negated loop condition for the false-branch). Return the exit node with its computed environment (Line 30).
\end{itemize}
...
SolQDebug computes sound over-approximations of variable ranges using interval domains
...
\subsection{Code Deletion and Modification}\label{sec:deletion}

While \S3.4.2--\S3.4.3 address incremental insertion,
editing also involves removing or modifying statements and blocks within a function body.

\smallskip
\noindent\textbf{Code Deletion.}
Algorithm~\ref{alg:deletion} handles code deletion by
validating syntax, reconnecting CFG edges, and triggering reinterpretation.
...
\noindent\textbf{Algorithm~\ref{alg:deletion}: Code Deletion.}
\begin{itemize}
  \item \textbf{Syntax validation (Lines 4--6):} Remove lines $[L_s, L_e]$ from $C$ to produce $C'$,
        then validate $C'$ using the Solidity compiler. If $C'$ is invalid, the deletion is rejected.
  \item \textbf{Boundary lookup (Line 7):} \texttt{FindBoundary} retrieves the first and last CFG nodes mapped to the deletion range
        via the line-to-node index $M$, then returns their unique predecessor and successor
        outside the range. For a simple statement, these are the adjacent nodes.
        For a compound block such as \texttt{if}/\texttt{else}, the start maps to a condition node whose predecessor
        is the node before the conditional, and the end maps to a join node whose successor is the next statement.
  \item \textbf{Edge removal and reconnection (Lines 8--11):} Detach the edges connecting the deleted region to the surrounding CFG,
        then add a direct edge from $pred$ to $succ$, bypassing the deleted region.
  \item \textbf{Node removal (Lines 12--14):} Remove all nodes mapped to lines in $[L_s, L_e]$ from the graph,
        along with their internal edges.
  \item \textbf{Index shift (Line 15):} Shift line-to-node mappings for all lines after $L_e$ backward
        by the number of deleted lines, maintaining index consistency.
  \item \textbf{Reinterpretation (Line 16):} Invoke Algorithm~\ref{alg:reinterpret} with $succ$ as the seed node,
        propagating updated abstract states through all affected program points.
\end{itemize}

\smallskip
\noindent\textbf{Code Modification.}
Code modification handles changes within a single statement,
such as altering a condition expression in an \texttt{if} statement or changing an assigned value.
The system updates the corresponding CFG node's statement in place and invokes
Algorithm~\ref{alg:reinterpret} from the modified node to propagate updated abstract states.

\FloatBarrier

\section{Evaluation}
To evaluate the benefits of SolQDebug in real-world development scenarios,
we structure our empirical analysis around three key questions:

\begin{itemize}
  \item \textbf{RQ1 -- Responsiveness}:\;%
        How much does SolQDebug reduce debugging latency compared to Remix?
  ...
  \item \textbf{RQ3 -- Loops}:\;%
        How does SolQDebug's analysis approach affect precision in loop structures?

  \item \textbf{RQ4 -- Pilot User Study}:\;%
        How do programmers perform when manually tracing variable values in Solidity functions?
\end{itemize}

To answer these questions, we conducted experiments under controlled conditions.

\subsection{Experimental Setup}
SolQDebug was evaluated on single-contract, single-transaction functions
to validate the contributions of incremental CFG construction and
annotation-guided abstract interpretation. Multi-contract analysis is left
as future work.
...
SolQDebug is implemented in Python~3.10 and operates directly on Solidity
...
3,344 Solidity contracts targeting compiler version 0.8.0 or later. To ensure representative
coverage across contract sizes, contracts smaller than 4~KB (2,142 samples) were first excluded,
as they typically contain minimal logic unsuitable for debugging analysis. From the remaining
...
\noindent The hierarchical filtering ensured that each function was counted in exactly one exclusion category. As shown in Table~\ref{tab:filtering-stats}, external dependencies dominate (70.8\%), reflecting the highly modular nature of modern DeFi contracts. After filtering, 142 candidate functions (9.3\%) remain suitable for debugging analysis.
...
\begin{itemize}
  \item \textbf{Computational complexity}---complex arithmetic patterns with
        chained computations across multiple statements, where value ranges
        are hard to predict manually.
...
While abstract interpretation techniques such as demanded abstract interpretation~\citep{daig} have been proposed for other languages,
these focus on optimizing static analysis pipelines rather than supporting interactive debugging.
Remix IDE, in contrast, provides debugging functionality for tracing execution flows with user-specified inputs, aligning with SolQDebug's target task.
We therefore selected Remix as our baseline, as it is the most widely used Solidity IDE that provides comparable debugging functionality.
...
For SolQDebug, it represents the time from a code modification to the display of updated variable information.
...
However, because each distinct input value requires a separate compile-deploy-execute cycle in Remix, the prohibitive time cost limited Remix measurements to $\Delta = 0$.
...
For SolQDebug, although designed for interactive use within a Solidity editor, all experiments simulate this behavior in a controlled scripting environment.
For each function, a sequence of incremental edits and annotations was reconstructed that mimics realistic developer activity.
...
\begin{table}[t]
\centering
\caption{Statistical comparison of debugging workflow latency between SolQDebug and Remix (test-case width $\Delta = 0$, 5-run means, $n=30$ contracts).}
...
Figure~\ref{fig:rq1-responsiveness} illustrates the latency comparison: function complexity measured by bytecode operation count (ByteOp count, x-axis), test-case width $\Delta$ (y-axis), and latency in seconds (z-axis).
Remix's latency scales linearly with both dimensions, reaching an estimated 256--1,287 seconds at $\Delta = 10$, while SolQDebug maintains sub-second latency across all conditions (0.02--3.01s at $\Delta = 10$).
...
Under this condition, Remix's debugging workflow latency ranged from 25.6 to 128.7 seconds (mean: 54.62s, median: 51.68s), while SolQDebug completed analysis in 0.02--1.88 seconds (mean: 0.24s, median: 0.09s).
...
while SolQDebug provides immediate statement-level abstract summaries during editing.

\medskip
\noindent\fbox{%
\begin{minipage}{0.96\linewidth}
\textbf{Answer to RQ1:}
SolQDebug achieves sub-second feedback (median 0.09s) compared to Remix's compile-deploy-debug cycle (median 51.68s), with statistical significance ($p < 0.001$).
\end{minipage}%
}
...
$[500, 500+\Delta]$). F90 was measured, defined as the 90th percentile of the inflation factor
...
the extreme products remain closer to the center of the resulting interval, limiting excessive interval expansion.
...
The analysis produces a tight interval \texttt{i} $\in [1,5]$ for the loop variable,
...
does not appear in the fixpoint output because \texttt{break} exits the loop before reaching the back edge.
Nevertheless, when combined with debugging annotations,
its value is captured at the loop exit point (a join of all exit paths) and remains available for subsequent analysis.
...
However, SolQDebug conservatively models uninitialized variables as $\top$ (unknown), causing \texttt{total} to remain $\top = [0, 2^{256}-1]$ regardless of how precisely the loop bound is known.
...
Once the widening threshold is exceeded, SolQDebug widens \texttt{resultIndex} to $[0, \infty]$.
...
\begin{minipage}{0.96\linewidth}
\textbf{Answer to RQ3:}
SolQDebug improves loop analysis precision for constant-bounded and annotation-enabled dynamic loops.
...
\subsection{RQ4 - Pilot User Study}

To measure the practical benefits of automated debugging assistance for users,
we conducted a preliminary study with 21 participants.
Participants were presented with five functions based on the benchmark dataset (Table~\ref{tab:benchmark-dataset}), selected for their complex arithmetic patterns.
For each function, participants were given specific input values and asked to calculate the return value.
The study was hosted on Cognition.run~\citep{cognition}, which automatically recorded response times.
Table~\ref{tab:rq4-demographics} summarizes participant demographics:
the majority had more than 5 years of programming experience and intermediate or advanced Solidity experience.

\begin{table}[!ht]
\centering
\caption{Participant Demographics}
\label{tab:rq4-demographics}
\small
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}llr@{}}
\toprule
\textbf{Category} & \textbf{Level} & \textbf{Count} \\
\midrule
\multirow{3}{*}{Programming Exp.}
  & 1--2 years & 1 (4.8\%) \\
  & 3--5 years & 5 (23.8\%) \\
  & More than 5 years & 15 (71.4\%) \\
\midrule
\multirow{4}{*}{Solidity Exp.}
  & None & 1 (4.8\%) \\
  & Beginner & 5 (23.8\%) \\
  & Intermediate & 7 (33.3\%) \\
  & Advanced & 8 (38.1\%) \\
...
\begin{table}[!ht]
\centering
\caption{Tool Usage and Accuracy}
\label{tab:rq4-tools}
\small
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lrrr@{}}
\toprule
\textbf{Tool} & \textbf{Users} & \textbf{Accuracy} & \textbf{Median Time} \\
\midrule
LLM (ChatGPT, Claude, etc.) & 11 (52.4\%) & 98.2\% & 43.1s \\
Non-LLM & 10 (47.6\%) & 62.0\% & 125.9s \\
\midrule
\textbf{Total} & \textbf{21 (100\%)} & \textbf{81.0\%} & \textbf{66.4s} \\
\bottomrule
\end{tabular*}
\end{table}

The overall accuracy across all problems was 81.0\% (85/105 responses correct),
with a median response time of 66.4 seconds per problem.
As shown in Table~\ref{tab:rq4-tools}, participants who did not use LLM assistants (using a calculator, mental calculation, or pen and paper) achieved 62\% accuracy (31/50) with a median time of 125.9 seconds,
indicating that manual value tracing is demanding even for experienced programmers.
In contrast, slightly over half of the participants (52.4\%) relied on LLM assistants such as ChatGPT or Claude, achieving 98.2\% accuracy.
However, even with LLM assistance, participants spent a median of 43.1 seconds per problem,
as the workflow required copying code to the LLM interface, specifying input values, and interpreting responses.
In comparison, SolQDebug analyzed the same functions in 0.08--0.18 seconds (mean: 0.12s, median: 0.10s).
These results highlight the need for integrated debugging assistance that provides immediate feedback within the development environment.

\medskip
\noindent\fbox{%
\begin{minipage}{0.96\linewidth}
\textbf{Answer to RQ4:}
Without LLM assistance, participants achieved only 62\% accuracy.
While LLMs improved accuracy to 98.2\%, they require external tool workflows.
SolQDebug addresses this by providing integrated, edit-time feedback within the development environment.
\end{minipage}%
}
...
Additionally, while a pilot study on value tracing was conducted (RQ4),
participants' experience levels were self-reported without independent verification.
A formal usability study evaluating developers' experience with SolQDebug in realistic workflows has not yet been performed.

\smallskip
\noindent\textbf{Construct Validity.}
We compared SolQDebug only against Remix IDE,
...
Unlike conventional step-through debuggers that trace concrete execution, SolQDebug provides abstract value summaries that characterize possible behaviors across symbolic interval inputs.

\smallskip
\noindent\textbf{Why Abstract Interpretation?}
Compared to other static analysis techniques, dynamic analysis, and machine learning techniques,
AI provides two properties essential for interactive debugging:
\begin{itemize}
  \item \textbf{Termination.} AI enforces convergence through widening at loops and $\sqcup$-joins at control-flow merges, guaranteeing bounded analysis time regardless of program complexity.
...
\smallskip
\noindent\textbf{Why Flow-Sensitive but Path-Insensitive?}
SolQDebug provides an interval for each variable at each program point.
This design reflects path-insensitive analysis, which merges abstract states via $\sqcup$ at join points.
Path-sensitive analysis would require presenting per-path results at each program point,
forcing developers to examine each path individually and mentally reconstruct the conditions that lead to each result.
This would reintroduce the manual value tracing burden that the tool is designed to eliminate.
While $\sqcup$-merging can introduce over-approximation when multiple branches are feasible,
condition-based pruning at branch points mitigates this over-approximation by setting the abstract state along infeasible branches to $\bot$,
and developers can further narrow input ranges through annotations to control precision where needed.

\smallskip
\noindent\textbf{Precision-Responsiveness Tradeoff.}
AI with the interval domain prioritizes responsiveness over precision. This tradeoff is justified
...
reverting the workflow to the traditional compile-deploy-debug cycle.
SolQDebug achieves millisecond-scale feedback latency (RQ1), preserving the interactive workflow.
Although AI's precision is inherently conservative, this limitation is mitigated
...
1. Basic principle
& All operators & Annotate one operand as interval (e.g., \texttt{[100,110]}), others as singleton (e.g., \texttt{[100,100]}). & Minimizes interval expansion from combinatorial endpoint calculations. \\[0.5em]
\hdashline\noalign{\vskip 1em}
\multirow[t]{5}{*}{\parbox{2.4cm}{2. Multiple\\intervals needed}}
& \texttt{*}, \texttt{/} & Use overlapping ranges (e.g., \texttt{[100,110]} and \texttt{[100,115]}). & Disjoint ranges maximize spread of endpoint products. \\[0.5em]
...
& \texttt{-} (\texttt{uint}) & Use disjoint ranges where minuend $>$ subtrahend (e.g., \texttt{[100,110] - [50,70]}). & Overlapping ranges risk underflow, degrading precision. \\[0.5em]
...
& Initialization & Initialize local variables explicitly (e.g., \texttt{uint x = 0;}). & Uninitialized locals start as $\top$; annotations cannot recover precision. \\[0.5em]
...
The experimental findings from RQ2 and RQ3 yield actionable strategies for developers to maximize analysis precision. Table~\ref{tab:guidelines} synthesizes these findings into a unified framework that includes annotation strategies for both arithmetic and loops, as well as code design patterns specifically for loop constructs.
    ...
    The current implementation is limited to single-contract, single-transaction scenarios and supports a subset of Solidity language features.
    ...
    Inline assembly allows developers to write low-level Yul code within Solidity, bypassing the type system and enabling low-level optimizations.
    ...
    As a conservative baseline, assembly blocks could be treated by assigning $\top$ to all variables potentially modified within the block.
    ...
    Inter-contract calls, inheritance hierarchies, and multi-transaction workflows are out of scope.
    ...
    This avoids full inlining while preserving soundness. Multi-transaction analysis would additionally require persisting abstract states across transaction boundaries.

    \smallskip
    \noindent\textbf{User Experience Validation.}
    While a pilot study on value tracing was conducted (RQ4),
    a formal usability study evaluating developers' experience with SolQDebug in realistic workflows has not yet been performed.
    ...
    Additionally, the same study found that practitioners prefer lightweight, development-integrated tools over standalone analyzers.
    SolQDebug's design aligns with this preference by providing immediate feedback during editing.
    Since SolQDebug currently addresses only single-contract, single-transaction scenarios without full IDE integration,
    comprehensive usability evaluation is planned once multi-contract support and IDE integration are implemented.
...
\section{Related Work}
    ...
    \subsection{Solidity IDEs and Debuggers}
        ...
        through EVM instructions and inspect the stack, memory, and storage. Hardhat~\citep{hardhat}
        is a Node.js-based framework that bundles a local Ethereum network with built-in Solidity compilation support. Third-party Visual Studio Code extensions integrate with Hardhat to provide step-by-step debugging of transaction execution. Forge~\citep{forge}, part of the Foundry toolchain, is a command-line tool oriented toward fast,
        reproducible unit testing. The command \texttt{forge test} spins up a local EVM instance,
        ...
        By contrast, SolQDebug targets pre-deployment authoring, accepts partial fragments
    ...
    \subsection{Solidity Vulnerability Detection and Verification}
        ...
        \citep{securify,slither}, symbolic-execution tools such as Mythril \citep{mythril},
        knowledge-graph approaches such as Solidet \citep{solidet}, and CFG refinement
        techniques such as Ethersolve \citep{ethersolve}.
        ...
        These approaches have substantially advanced the security analysis
        of smart contracts. However, they target complete programs and focus on detecting known vulnerability patterns rather than providing edit-time feedback. In contrast, SolQDebug
    ...
    \subsection{Solidity-Specific Abstract Interpretation Frameworks}
        AI is a well-established theory for static analysis and has been
        ...
        Matrix (DBM) domain to generate state invariants and detect reentrancy vulnerabilities,
        including the pattern exploited in the DAO attack~\citep{DBM, dao}. However, both approaches operate on fully written
        contracts and provide no support for line-by-line interpretation or developer interaction
        within an IDE.

        SolQDebug builds upon these AI frameworks but targets interactive debugging
        rather than invariant generation or information flow analysis. Unlike these approaches, which analyze complete contracts, SolQDebug incrementally updates
    ...
    \subsection{Interactive Abstract Interpretation for Traditional Languages}
        Beyond Solidity, the broader programming languages community has explored
        making abstract interpretation interactive. Recent work has integrated static analysis
        directly into IDEs for C, Java, and other traditional languages, delivering live feedback
        during editing \citep{daig, ds, iac, iaj, fap}. While these systems share SolQDebug's
        ...
        A follow-up study by \citet{ds} generalized this to procedure summaries, enabling inter-procedural
        reuse.
        \citet{iac} extended Goblint with incremental support for multithreaded C, selectively
        recomputing only affected facts and maintaining IDE-level responsiveness.
        ...
        in traditional languages, SolQDebug represents, to the best of our knowledge, the first application of this paradigm
...
\section{Conclusion}
    ...
    The analysis also revealed how annotation patterns affect interval expansion in arithmetic operations and loop constructs, which were synthesized into practical guidelines for developers.
    Additionally, a pilot study confirmed that manual value tracing is demanding for programmers, validating the practical need for such assistance.
    ...
    Future work includes extending language coverage to inline assembly, supporting inter-contract calls through summary-based analysis, and enabling multi-transaction workflows.
...
This work was supported by the Institute of Information \& communications Technology Planning \& Evaluation (IITP) grants funded by the Korean government (MSIT) (RS-2021-II210177, High Assurance of Smart Contract for Secure Software Development Life Cycle) and the Technology Development Program (RS-2025-25455127) funded by the Ministry of SMEs and Startups (MSS, Korea).
...
\bibitem[Cognition.run(2026)]{cognition}
Cognition.run: \url{https://www.cognition.run/} (2026). Accessed January 2026
...
This appendix provides the complete CFG construction patterns for SolQDebug's incremental CFG builder.
