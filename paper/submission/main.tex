...
\section{Introduction}\label{sec1}
    ...
    This paper presented SolQDebug, an edit-time debugging assistant for Solidity designed for interactive analysis within a single contract/transaction context,
    ...
    To achieve this goal, SolQDebug built on two core ideas.
...
\FloatBarrier
    ...
    \subsection{Proposed Approach Overview}
        ...
        SolQDebug replaces this round trip by performing both parsing and abstract interpretation directly inside the Solidity Editor.
        ...
        Both approaches disrupt iteration. SolQDebug introduces batch annotations, enabling developers to vary inputs or contract state to observe control-flow outcomes.
...
\section{The design of SolQDebug}
    ...
    \subsection{System Architecture}
        ...
        where values are defined, assigned, returned, or changed through loop iterations by comparing pre-loop and post-loop abstract states.
...
Code modification is handled by decomposing the operation into a deletion followed by an insertion.
When the modified code spans the same number of lines as the original,
the system updates the corresponding CFG node statements in place and invokes
Algorithm~\ref{alg:reinterpret} from the modified node.
When the line count differs, the system first applies Algorithm~\ref{alg:deletion}
to remove the original lines, then performs incremental insertion (\S3.4.2--\S3.4.3) for the replacement code.
Structural refactoring operations such as wrapping existing code in an \texttt{if}/\texttt{else} block
are similarly achieved through a sequence of deletion and insertion steps,
as each step is independently processed by the incremental analysis pipeline.
...
However, SolQDebug conservatively models uninitialized variables as $\top$ (unknown), causing \texttt{total} to remain $\top = [0, 2^{256}-1]$ regardless of the precise loop bound.
...
For each function, input values were provided with the task of calculating the return value.
...
As shown in Table~\ref{tab:rq4-tools}, participants who did not use LLM assistants (using calculator, mental calculation, or pen and paper) achieved 62\% accuracy (31/50) with a median time of 125.9 seconds,
indicating that manual value tracing is demanding even for experienced programmers.
In contrast, the majority (52.4\%) relied on LLM assistants such as ChatGPT or Claude, achieving 98.2\% accuracy.
However, even with AI assistance, participants spent a median of 43.1 seconds per problem,
as the workflow requires copying code to the AI interface, specifying input values, and interpreting responses.
...
SolQDebug provides an interval per program point.
This design naturally corresponds to path-insensitive merging via $\sqcup$ at join points.
...
condition-based pruning at branch points reduces this by filtering infeasible branches to $\bot$,
...
AI with the interval domain prioritizes responsiveness over precision, a tradeoff justified
...
SolQDebug achieved millisecond-scale feedback latency (RQ1), keeping developers in flow.
...
\section{Related Works}
    ...
    \subsection{Solidity Vulnerability Detection and Verification}
        ...
        for smart contracts. However, they target complete programs and focus on vulnerability detection rather than providing edit-time feedback. In contrast, SolQDebug
    ...
    \subsection{Solidity-Specific Abstract Interpretation Frameworks}
        ...
        rather than invariant generation or information flow analysis. Unlike these approaches that analyze complete contracts, SolQDebug incrementally updates
    ...
    \subsection{Interactive Abstract Interpretation for Traditional Languages}
        ...
        in traditional languages, SolQDebug represents the first application of this paradigm
