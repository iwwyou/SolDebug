...
\begin{itemize}
  ...
  \item \textbf{Require/assert statement} creates a \texttt{condition node} with the true edge connecting to a continuation node and the false edge pointing to the \texttt{error exit node}.
        The environment propagates with pruning based on the predicate to the continuation or error exit node (Figure~\ref{fig:new-require}).
\end{itemize}
