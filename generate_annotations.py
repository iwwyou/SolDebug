    ...
    for line in lines:
        ...
        if line.startswith('SV :') or line.startswith('LV :'):
            ...
            for var_name in var_names:
                # Clean up variable name (remove array indices, etc.)
                clean_name = re.sub(r'\[.*?\]', '', var_name).strip()
                if clean_name:
                    # Generate default values based on variable type
                    default_values = generate_default_values(clean_name, var_type)
                    variables.append((var_type, clean_name, default_values))
